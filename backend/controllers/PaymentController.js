...
class PaymentController {
    async paymentProcess(req, res, next) {
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            mode: 'payment',
            success_url: `${process.env.CLIENT ? process.env.CLIENT : ""}/user?session_id={CHECKOUT_SESSION_ID}`,
            cancel_url: `${process.env.CLIENT ? process.env.CLIENT : ""}/cart`,
        });
        res.json({ url: session.url });
    }
    async checkOutSession(request, response) {
        const secretKey = "sk_test_51NUjFaLITmQMXn5Wf95hA51iB40FxwDY5qifvKNAePJKs3J137DhS3kDm2LMxUSmL2I5bI3i9yr0zOyFSq3J0K9I00brEwkkXe"
        const sig = request.headers['stripe-signature'];
        //console.log('🚀 ~ file: PaymentController.js:80 ~ PaymentController ~ checkOutSession ~ sig:', sig);
        let event;
        try {
        ...
        //console.log('🚀 ~ file: PaymentController.js:97 ~ PaymentController ~ checkOutSession ~ event.type:', event.type);
        // Handle the event
        switch (event.type) {
                ...
                customer.forEach(async (ctr) => {
                    try {
                        ...
                        if (product) {
                            ...
                            await ProductModel.findByIdAndUpdate(ctr._id, { stock }, { new: true });
                        }
                    } catch (error) {
